...
## 2024-10-27 - Form Accessibility Enhancements
**Learning:** Wrapping form inputs inside a `<fieldset disabled={isLoading}>` natively disables the entire form during async requests instead of manually disabling individual inputs and buttons. This automatically removes them from the tab order and provides better semantic context.
**Action:** Use the `<fieldset disabled={isLoading}>` pattern to encapsulate and disable form content when building async forms.
