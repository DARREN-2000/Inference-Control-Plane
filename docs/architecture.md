...
Laminar is designed as a high-throughput, low-latency API gateway that sits securely between your applications and various AI foundational models. It is built on a modern, asynchronous Python stack to handle thousands of concurrent streaming connections efficiently.

## Core Components

The architecture consists of four primary components:

1. **The Proxy API (Python/FastAPI):** The heart of Laminar. It handles incoming HTTP requests, performs authentication, enforces rate limits, queries the cache, routes the request to the upstream provider, and streams the response back.
...
When a client application makes a request to Laminar, it goes through a strict pipeline designed to add less than 3 milliseconds of overhead.

```mermaid
sequenceDiagram
    participant Client
    participant API as Laminar API
    participant Redis as Redis (Cache/Limits)
    participant Model as LLM Provider
    participant DB as Postgres (Logs)

    Client->>API: POST /v1/chat/completions (Bearer: sk-laminar...)
...
1. **Authentication:** The `sk-laminar` key is validated. To ensure sub-millisecond validation, API keys are cached in Redis.
...
1. **Policy Evaluation:** Laminar determines the primary model and fallback chain based on the tenant's configuration or request headers.
...
3. **Streaming:** As the provider streams tokens back, Laminar immediately yields them to the client connection, ensuring zero perceived latency overhead during generation.
...
Laminar is container-native and designed to be deployed horizontally.
...
For globally distributed applications, Laminar can be deployed across multiple geographic regions to reduce latency.
...
- **Connection Management:** Because AI requests can take tens of seconds, standard synchronous Python web servers block quickly. Laminar uses ASGI (Uvicorn) and `asyncio`, allowing a single API worker to handle thousands of concurrent long-lived streaming connections without thread exhaustion.
