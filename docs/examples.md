...
This document provides practical, real-world examples of how to utilize Laminar's capabilities to solve common AI engineering challenges.
...
By using `extra_body`, you instruct Laminar on how to handle failures.

```python
from openai import OpenAI

client = OpenAI(base_url="http://laminar-host:8000/v1", api_key="sk-laminar...")
...
try:
    response = client.chat.completions.create(
        ...
        extra_body={
            "laminar_fallback_models": ["claude-3-5-sonnet", "gpt-3.5-turbo"]
        }
    )
...
*What happens in Laminar:*
1. Laminar calls OpenAI `gpt-4o`.
2. OpenAI returns a 500 error.
3. Laminar catches the error, looks at `laminar_fallback_models`, and instantly retries the exact same prompt against Anthropic's `claude-3-5-sonnet`.
...
**Implementation:**
Ensure `CACHE_ENABLED=true` is set in your Laminar `.env`.

```bash
# Request 1 (Takes 1.5s, Costs $0.001)
curl -X POST "http://localhost:8000/v1/generate" \
  -H "Authorization: Bearer sk-laminar..." \
...
```
To explicitly force the LLM to generate a new response and ignore the cache, append `"laminar_cache_bypass": true` to the request payload.
...
**Implementation:**
In your Laminar environment, set `USER_RATE_LIMIT_PER_MINUTE=10`.

Then, in your application, ensure you pass the `user` field with the end-user's ID.

```typescript
import OpenAI from 'openai';
const client = new OpenAI({ baseURL: 'http://localhost:8000/v1', apiKey: 'sk-laminar...' });
...
app.post('/ask-ai', async (req, res) => {
  ...
  try {
    ...
    if (error.status === 429) {
      res.status(429).json({ error: "You are making too many requests. Please slow down." });
    }
  }
});
```

If `usr_abc123` makes 11 requests in a minute, Laminar will reject the 11th request locally at the Redis layer, returning a 429 without ever calling OpenAI.
...
**Implementation:**
Create separate Laminar API keys for each department. Laminar treats each API key as a distinct `tenant_id`.

When departments make requests using their respective keys, Laminar tags all logs in PostgreSQL with that `tenant_id`.

You can then query the Laminar Admin API (or view the Dashboard) to get usage metrics:
