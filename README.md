...
*For a detailed look at the internal data flows, see our [Architecture Deep Dive](docs/architecture.md).*
...
*For highly-available multi-region setups and connection pooler (PgBouncer) configurations, consult the [Deployment Guide](docs/deployment.md).*
...
*Note: Benchmarks conducted via `wrk` on AWS c6g.large instances simulating concurrent SSE stream handling.*
