...
with open("src/inference_control_plane/api/routes.py") as f:
    content = f.read()

new_content = re.sub(
    r'@router\.get\("/dashboard/metrics".*?(?=@router\.get\("/health/live"\))',
...
async def dashboard_metrics(
    ...
    stmt_hits = select(func.count(RequestLog.id)).where(RequestLog.created_at >= yesterday, RequestLog.cache_hit == True)
